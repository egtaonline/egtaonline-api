...
_PARSER = argparse.ArgumentParser(prog='eo', description="""Command line access
                                  to egta online apis""")
...
_PARSER_SIM.add_argument('--json', '-j', metavar='json-file', nargs='?',
                         type=argparse.FileType('r'),
                         default=argparse.SUPPRESS, const=sys.stdin,
                         help="""Modify simulator using the specified json
file. By default this will add all of the roles and strategies in the json
file. If `delete` is specified, this will remove only the strategies specified
in the file. `-` can be used to read from stdin. (default: stdin)""")
...
_PARSER_SIM.add_argument('--delete', '-d', action='store_true',
                         ...
                         instead of addition""")
_PARSER_SIM.add_argument('--zip', '-z', action='store_true',
                         help="""Download simulator zip file to stdout.""")
...
_PARSER_GAME.add_argument('--json', '-j', metavar='json-file', nargs='?',
                          ...
                          strategies in the json file. If `delete` is
                          specified, this will remove only the strategies
                          specified in the file. (default: stdin)""")
...
_PARSER_GRAN.add_argument('--structure', action='store_true', help="""Return
                          game information but no profile information.
                          (default)""")
_PARSER_GRAN.add_argument('--summary', action='store_true', help="""Return
                          profiles with aggregated payoffs.""")
_PARSER_GRAN.add_argument('--observations', action='store_true', help="""Return
                          profiles with observation data.""")
_PARSER_GRAN.add_argument('--full', action='store_true', help="""Return
                          all collected payoff data.""")

_PARSER_SCHED = _SUBPARSERS.add_parser('sched', description="""Operate on EGTA
                                       Online schedulers.""")
_PARSER_SCHED.add_argument('sched_id', nargs='?', metavar='sched-id',
...
def main():
    ...
    if args.auth_string is None:
        if path.isfile(args.auth_file):
            ...
            sys.stderr.write(textwrap.fill(' '.join(
                '''This script needs an authorization token to access EGTA
                Online. By default, this script looks for file with your
                authorization token at "{}". See `egta --help` for other ways
                to specify your authorization token.'''
                .format(_DEF_AUTH).split())))
            sys.stderr.write('\n')
            sys.exit(1)
